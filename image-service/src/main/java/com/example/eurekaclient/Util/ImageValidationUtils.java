...
public class ImageValidationUtils {
    ...
    public static void validateImage(byte[] imageBytes) throws InvalidImageException {
        ...
        if (imageBytes.length > MAX_FILE_SIZE) {
            throw new InvalidImageException("Image file is too large. Maximum allowed size is " + MAX_FILE_SIZE / (1024 * 1024) + "MB");
        }

        String contentType = detectContentType(imageBytes);
        if (!ALLOWED_CONTENT_TYPES.contains(contentType)) {
            throw new InvalidImageException("Invalid image type. Allowed types are: " + String.join(", ", ALLOWED_CONTENT_TYPES));
        }
    ...
    private static void validateResolution(byte[] imageBytes) throws InvalidImageException {
        try (ByteArrayInputStream bais = new ByteArrayInputStream(imageBytes)) {
            ...
            if (width < MIN_WIDTH || height < MIN_HEIGHT) {
                throw new InvalidImageException("Image resolution is too small. Minimum allowed resolution is " + MIN_WIDTH + "x" + MIN_HEIGHT);
            }

            if (width > MAX_WIDTH || height > MAX_HEIGHT) {
                throw new InvalidImageException("Image resolution is too large. Maximum allowed resolution is " + MAX_WIDTH + "x" + MAX_HEIGHT);
            }
        } catch (IOException e) {
